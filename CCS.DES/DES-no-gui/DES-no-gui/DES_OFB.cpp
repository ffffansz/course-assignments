...
void DES_OFB::setPltxt(shared_ptr<std::string>& pPltxt)
{
	pPltxt_ = pPltxt;
	return;
}

void DES_OFB::setCitxt(shared_ptr<std::string>& pCitxt)
{
	pCitxt_ = pCitxt;
	return;
}

shared_ptr<std::string> DES_OFB::getCitxt()
{
	return pCitxt_;
}

shared_ptr<std::string> DES_OFB::getPltxt()
{
	return pPltxt_;
}

shared_ptr<dynamic_bitset<>> DES_OFB::getCiFileBits()
{
	return pCiFileBits_;
}

shared_ptr<dynamic_bitset<>> DES_OFB::getPlFileBits()
{
	return pPlFileBits_;
}
...
DES_OFB::DES_OFB(const dynamic_bitset<>& key, const dynamic_bitset<>& IV, const shared_ptr<std::string>& pPltxt, const shared_ptr<std::string>& pCitxt)
...
DES_OFB::DES_OFB(const dynamic_bitset<>& key, const dynamic_bitset<>& IV, const shared_ptr<dynamic_bitset<>>& pPlFileBits, const shared_ptr<dynamic_bitset<>>& pCiFileBits)
...
void DES_OFB::encry()
{
	...
	for (size_t i = 0; i < pPlbitsets_->size(); i++) {
		...
		shiftReg_ = des::util::padLeftShift(shiftReg_, partRet.getCipherBits(), (*pPlbitsets_)[i]->size());
	}
...
void DES_OFB::decry()
{
	...
	shiftReg_ = des::util::padLeftShift(shiftReg_, firstPartRet.getCipherBits(), (*pCibitsets_)[0]->size());
	for (size_t i = 1; i < pCibitsets_->size(); i++) {
		...
		shiftReg_ = des::util::padLeftShift(shiftReg_, partRet.getCipherBits(), (*pCibitsets_)[i]->size());
	}
	assert(pPlbitsets_->size() != 0);
